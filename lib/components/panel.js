...
async function buyAdpLegalOnText(ctx, session) {
...
Pilih produk admin panel`,
      {
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "Admin Panel - 10000",
                callback_data: "buy_adp_legals"
              }
            ]
          ]
        }
  ...
  if (ctx.callbackQuery) {
    ...
    if (data !== "admin_panel") {
      return ctx.answerCbQuery("Produk tidak valid")
    }
    ...
    return await buyAdminPanel(ctx, session, Markup)
  }
}

async function buyAdpbiasaOnText(ctx, session) {
