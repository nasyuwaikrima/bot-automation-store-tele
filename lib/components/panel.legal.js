...
async function buyPanelLegal(ctx, session, data, Markup) {
...
const panelConfig = require("./panel.legal.json")
...
📌 Status: ${statusText}`, {
  ...
  if (check.status === "SETTLED") {
    ...
    const createUser = await fetch(`${urlLegal}/api/application/users`, {
      method: "POST",
      headers: {
    ...
    const createServer = await fetch(`${urlLegal}/api/application/users`, {
      method: "POST",
      headers: {
        "Accept": "application/json",
        "Content-Type": "application/json",
